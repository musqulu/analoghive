...
describe("useTimer", () => {
  ...
  it("resetTimer returns to idle and restores first-step display time", () => {
    const { result } = createTimer()
    act(() => result.current.startTimer("dev"))
  ...
  it("does not fire onSessionReset when reset clears a post-dev step", () => {
    ...
    const times: ProcessTimes = { dev: 0.05, stop: 1, fix: 5, wash: 5 }
    const { result } = createTimer({
    ...
    expect(result.current.postDevRollPending).toBe(true)
  })

  it("clears postDevRollPending after wash completes naturally", () => {
    ...
    act(() => jest.advanceTimersByTime(3500))
    act(() => jest.advanceTimersByTime(3500))
    expect(result.current.postDevRollPending).toBe(false)
  })
